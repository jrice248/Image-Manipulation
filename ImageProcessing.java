...
public class ImageProcessing {
	public static void main(String[] args) {
        // Need to create way for image uploading
		...
		// Need to create UI to let you select what imnage manipulation they want to do
        ...
        twoDToImage(blankImg, "./blank_img.jpg");
        ...
        int[][] rectangleImg = paintRectangle(blankImg, 100, 200, 100, 100, getColorIntValFromRGBA(rgba));
	...
	public static int[][] stretchHorizontally(int[][] imageTwoD) {
        ...
        for (int i = 0; i < imageTwoD.length; i++) {
            for (int j = 0; j < imageTwoD[i].length; j++) {
                int it = j * 2;
                stretchedImageTwoD[i][it] = imageTwoD[i][j];
                stretchedImageTwoD[i][it + 1] = imageTwoD[i][j];
            }
    }
	...
	public static int[][] shrinkVertically(int[][] imageTwoD) {
        int[][] shrinkedImageTwoD = new int[imageTwoD.length / 2][imageTwoD[0].length];

        for (int i = 0; i < imageTwoD[0].length; i++) {
            for (int j = 0; j < imageTwoD.length-1; j += 2) {
                shrinkedImageTwoD[j / 2][i] = imageTwoD[j][i];
            }
    }
	...
	public static int[][] invertImage(int[][] imageTwoD) {
        int[][] invertedImageTwoD = new int[imageTwoD.length][imageTwoD[0].length];
        
        for (int i = 0; i < imageTwoD.length; i++) {
                for (int j = 0; j < imageTwoD[i].length; j++) {
                    invertedImageTwoD[i][j] = imageTwoD[(imageTwoD.length - 1) - i][(imageTwoD[i].length - 1) - j];
                }
        }

        return invertedImageTwoD;
	}
